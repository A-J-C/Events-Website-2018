...
const href = "http://127.0.0.1:8090";																						// start url for server
...
app.get(BASE + '/index', function(req, resp) {																	// when accessing root
	console.log("index.html requested by: "+req.connection.remoteAddress);				// log request
	resp.status(200).sendFile(BASE + '/index.html');															// serve index page
});

/* GET BASE/admin */
app.get(BASE + '/admin', function(req, resp) {																	// when accessing root
	console.log("admin.html requested by: "+req.connection.remoteAddress);				// log request
...
app.get(BASE + '/venues', function(req, resp) {
	...
	console.log("venues requested by: "+req.connection.remoteAddress);
...
app.get(BASE + '/events/search', function(req, resp) {
	/* Both are optional paramaters, returns all if no paramaters */
	console.log("events search requested by: "+ req.connection.remoteAddress);
...
app.get(BASE + '/events/get/:event_id', function(req, resp) {
	...
	console.log("events get requested by: "+req.connection.remoteAddress);
	var eventID = cleanse(req.params.event_id);																		// extract event id
		...
		db.all("SELECT * FROM events WHERE event_id LIKE '"+eventID+"'", function(err, respDB) {
			if (respDB.length == 0)
				resp.status(400).send({"error" : "no such event"});											// send error if event not found
				 ...
				 var sEvent = respDB[0];
				 db.all("SELECT * FROM venues WHERE venue_id LIKE '"+sEvent.venue_id+"'", function(err, respDB2) {
					 var venue = respDB2[0];
					 delete sEvent.venue_id;
					 sEvent["venue"] = venue;
					 resp.set({"Content-Type": "application/json", 												// define header, stops results being cached
		 			"Cache-Control": "private,no-cache,no-store,must-revalidate"});
		 			resp.status(200).send(sEvent); 																				// send response back
				});
			 }
		});
	}
...
app.post(BASE + '/venues/add', function (req, resp) {
	...
	console.log("venues add requested by: "+req.connection.remoteAddress);
...
app.post(BASE + '/events/add', function (req, resp) {
	...
	console.log("events add requested by: "+req.connection.remoteAddress);
	var authToken = req.body.auth_token;																					// check if auth is given as query
	if (authToken == undefined && req.headers.cookie != undefined)								// if not get it from cookie
		authToken = (req.headers.cookie).split("=")[1];															// extract auth token from cookie
...
app.post(BASE + '/venues/del', function (req, resp) {
	/* four required params two optional */
	console.log("venues delete requested by: "+req.connection.remoteAddress);
	var authToken = req.body.auth_token;																					// check if auth is given as query
	if (authToken == undefined && req.headers.cookie != undefined)								// if not get it from cookie
		authToken = (req.headers.cookie).split("=")[1];															// extract auth token from cookie
	var ip = req.ip;																															// extract ip
...
app.post(BASE + '/events/del', function (req, resp) {
	/* four required params two optional */
	console.log("events delete requested by: "+req.connection.remoteAddress);
	var authToken = req.body.auth_token;																					// check if auth is given as query
	if (authToken == undefined && req.headers.cookie != undefined)								// if not get it from cookie
		authToken = (req.headers.cookie).split("=")[1];															// extract auth token from cookie
	var ip = req.ip;																															// extract ip
...
app.post(BASE + '/auth/del', function (req, resp) {
	console.log("auth del by: "+req.connection.remoteAddress);
	var authToken = req.body.auth_token;																					// check if auth is given as query
	if (authToken == undefined && req.headers.cookie != undefined)								// if not get it from cookie
		authToken = (req.headers.cookie).split("=")[1];															// extract auth token from cookie
...
app.listen(8090);																																// running on port 8090
